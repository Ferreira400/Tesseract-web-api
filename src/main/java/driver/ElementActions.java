...
public class ElementActions {
    ...
    public static WebElement getElement(By by, int timeout) {
        WebElement element = (WebElement)(new WebDriverWait(Drivers.getDriver(), timeout)).until(ExpectedConditions.elementToBeClickable(by));
        if (element == null) {
            Assertions.fail("Elemento não encontrado na página: " + element);
        }
        return element;
    }
    ...
    public static void waitElement(int timeout, By by) {
        ...
        if (time >= timeout * 1000 && !flag) {
            Assertions.fail("Elemento não encontrado na página");
        }
